...
// pooled-record reuse: once a record's promise fires (or ProduceSync returns for
// it), the caller may reset the record and return it to a pool.
//
// The bug they guard against is a data race between a still-in-flight hedge leg
...
// both tests are race-free once the client stops touching records after their
...
func TestWarpstreamClient_Produce_ReusedPooledRecordsAreRaceFree(t *testing.T) {
	...
	for i := 0; i < total; i++ {
		...
		c.Produce(context.Background(), rec, func(r *kgo.Record, err error) {
			...
			// The promise has fired for r, so the caller owns it again: reset and
			// return it to the pool. The client must not read r after this point
			// (an in-flight hedge leg doing so is the bug).
			*r = kgo.Record{}
			pool.Put(r)
			<-sem
			wg.Done()
